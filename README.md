...
GitHub Actions builds release assets only when a version tag is pushed.
...
- Windows installer assets on `windows-latest`
- Windows portable `.exe` on `windows-latest`
- Linux assets on `ubuntu-22.04`
- macOS Apple Silicon assets on `macos-latest`
- macOS Intel assets on `macos-latest`

Release notes are generated by GitHub from the commit history for the tag.
